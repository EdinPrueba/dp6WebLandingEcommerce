<template>
	<div class="details-main-container">
		<section class="nav">
			...
			<p :style="`color: ${globalColors.primary};`" class="order-head">
		...
		<section class="header">
			<div class="order-info">
				<div>
					...
					><span class="order-info-data">{{
						getValue('total', getOrderInfo)
					}}</span>
				</div>
				<div>
					<span class="label">Fecha de la Orden: </span
					><span class="order-info-data">{{
						getValue('createdAt', getOrderInfo)
					}}</span>
				</div>
				<div>
				...
				<div class="order-payment-wrapper">
					<div class="mb-2 delivery-address">
						<div v-if="isPaymentLink" class="payment-link-data">
							...
							<div>
								ID de transacción:
								<span class="label">{{ transactionPaymentLinkId }}</span>
							</div>
						</div>
						<div v-if="isPaymentez && !isPaymentLink" class="payment-link-data">
							<div class="link-container">
								Id transacción:
								<span class="label">{{ paymentezData.id }}</span>
							</div>
							<div>
								Código de transacción:
								<span class="label">{{ paymentezData.code }}</span>
							</div>
						</div>
						<section>
							<span v-if="flagPickUp === 1" class="label">
								Direccion de envio:
								<span class="order-info-data">{{
									getValue('customerAddress.addressLine1', getOrderInfo)
								}}</span>
							</span>
							<span v-else class="label">
								Direccion de recojo:
								<span class="order-info-data">{{
									getValue('warehouseName', getOrderInfo)
								}}</span>
							</span>
						</section>
						<div v-if="refund.exist" class="refund-container">
							...
							<div>
								...
								<h4 v-if="getCommerceData.phone">
									Teléfono: {{ getCommerceData.phone }}
								</h4>
							</div>
						</div>
						<section>
							<span v-if="getValue('comments', getOrderInfo)" class="label">
								Comentario:
								<span class="order-info-data"
									>{{ getValue('comments', getOrderInfo) }}
								</span>
							</span>
						</section>
					</div>
		...
		<transition-group
			name="go-right"
			tag="div"
			class="relative rating-container"
		>
			...
			<section class="table w-100" v-else :key="2">
				<responsive-table align-left :columns="columns" :rows="details">
					<template slot-scope="{ row }">
						<td class="row-product row-product-name">
							<div class="product-info-container">
								<img
									:src="row.productImage ? row.productImage : fallbackImage"
								...
								<div class="text-xs-left">
									<div class="product-name-mobile">
										<template v-if="row.productName.length > 24">
											<v-tooltip top>
												<h4 slot="activator" class="product-name">
													{{ row.productName | limitTo(23) }}
												</h4>
		...
		<modal-component v-model="showConfirm">
			<div class="slot-modal-wrapper">
				<section class="modal-content">
					<h3>
						Este enlace tiene una transacción de pago en proceso con referencia:
...
async function loadData() {
	await this.$store.dispatch('LOAD_ORDER_DETAILS', {
		context: this,
...
function transactionPaymentLinkId() {
	if (this.isPaymentLink) {
		const gTransactionId = this.getValue(
			'gatewayTransactionId',
			this.additionalInformation,
		);
		const refId = this.getValue(
...
function paymentState() {
	const paymentStateGateway = this.getValue(
		'paymentStateGateway',
		this.getOrderInfo,
	);
...
function refund() {
	const exist = getDeeper('gatewayAuthorizationResponse.refund.errors')(
		this.getOrderInfo,
	);
	const errors = {
		VALID_TIME_AUTOMATIC_REFUND_EXCEEDED:
			'Comuníquese con el administrador del ecommerce para su reembolso',
	};
	return {
		exist,
		text: errors[exist],
...
export default {
	...
	computed: {
		...mapGetters(['flagAddVoucher', 'getCommerceData', 'getOrderInfo']),
		details,
		flagPickUp,
		gatewayName,
		isDeposit,
